...
recode.IAWA <- function(strings, names = NULL, split = "-", code_variable = "v$", code_unsure = "\\?$",
  ...
  stopifnot(grepl(split, strings),
            if (!is.null(names)) length(names) == length(strings),
...
get.diag <- function(df, fac, level = nlevels(fac)-1, verbose = TRUE) {
  ...
  # be verbose
  # fname <- gsub("[()]", "", sub("(.*\\$)(.*)", "(\\2)", as.character(deparse(substitute(fac)))))
  fname <- paste(levels(fac), collapse = ",")
  if (verbose) {
    message("Searching diagnostic characters in ", fname, "\n  ", length(v.qual), " qualitative and ", length(v.quant), " quantitative variables at level ", level, ".")
  }
  ...
  if (verbose) {
    ...
    message("  found ", ndiag, " potentially useful (TPR >= 90%, FPR <= 10%) characters,\n  of which ", ndiaggrp, " are diagnostic (TPR = 100%, FPR = 0%) to distinguish ", " groups")
  }
