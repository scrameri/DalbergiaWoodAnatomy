...
message("Do weighted PCA / FAMD / PCAmix?")
scan()
...
p.rpca3 <- ggplot(data.frame(d.rpca$rotation)) +
  geom_segment(aes(x = 0, xend = -1*PC1, y = 0, yend = -1*PC2), arrow = arrow()) +
  geom_label(aes(x = -1*PC1, y = -1*PC2, label = rownames(d.rpca$rotation))) +
  lims(x = c(-.5,.5), y = c(-.5,.5)) +
  labs(x = paste0("PC 1 (", round(d.rpca$expl.var[1], 2), "%)"), y = paste0("PC 2 (", round(d.rpca$expl.var[2], 2), "%)")) +
  theme_bw() +
  ggtitle("Loadings of Wood Anatomy to PCA Axes")
...
#   theme_bw() +
#   ggtitle("Loadings of Wood Anatomy to PCA Axes")
p.rpca4 <- ggplot(data.frame(d.rpca$rotation)) +
  geom_segment(aes(x = 0, xend = 1*PC3, y = 0, yend = 1*PC4), arrow = arrow()) +
  geom_label(aes(x = 1*PC3, y = 1*PC4, label = rownames(d.rpca$rotation))) +
  lims(x = c(-.5,.5), y = c(-.5,.5)) +
  labs(x = paste0("PC 1 (", round(d.rpca$expl.var[3], 2), "%)"), y = paste0("PC 2 (", round(d.rpca$expl.var[4], 2), "%)")) +
  theme_bw() +
  ggtitle("Loadings of Wood Anatomy to PCA Axes")
